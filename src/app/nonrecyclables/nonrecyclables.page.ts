...
import { AddnonrecycPageModule } from '../addnonrecyc/addnonrecyc.module';
import { AddnonrecycPage } from '../addnonrecyc/addnonrecyc.page';

@Component({
  selector: 'app-nonrecyclables',
  templateUrl: './nonrecyclables.page.html',
  styleUrls: ['./nonrecyclables.page.scss'],
})
export class NonrecyclablesPage implements OnInit {
  ...
  constructor(private http: HttpClient, public modalController: ModalController,
    public toastController: ToastController, private datePipe: DatePipe, private commondata: CommondataService, private authService: AuthService, private activatedRoute: ActivatedRoute) {
    this.uri = JSON.parse(this.activatedRoute.snapshot.paramMap.get('id'));
  }
  ngOnInit() {
    console.log(this.activatedRoute, this.uri)
  ...
  callcenter(e) {
    ...
    this.http.get(`${Urls.CEN}/${e}/newWasteCollecteds?filter[where][type]=non-recyclable&filter[limit]=20&filter[order]=date%20DESC`).subscribe((res: any) => {
  ...
  rdate(e: any) {
    console.log(e)
    this.http.get(`${Urls.CEN}/${this.centerId}/newWasteCollecteds?filter[where][type]=non-recyclable&filter[where][date]=${e}`).subscribe((res: any) => {
      console.log(res)
      this.wList = res;
      console.log(this.wList)
      var tdTotal = 0;
