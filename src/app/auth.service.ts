...
export class AuthService {
  ...
  logout(u) {
    ...
    // });
    console.log(u);
    this.http.post<any>(`${Urls.LOGOUT}?access_token=${u}`, { "accessToken": u.id }).subscribe((res: any) => {
      console.log(res);
      ...
      .pipe(map(user => {
        ...
        if (role == 'Patient') {
          this.http.post<any>(`${Urls.DOCTOR}/${selecteddoctor}/patientLists`, {
            ...
            // .pipe(map(users => {
            //   console.log("PATIENT ADDED" + users.value);
            //   this.router.navigate(['/tabs/tab2', users.data.id]);
            // }))
            .subscribe(datas => {
              console.log(datas);
              console.log("PATIENT ADDED" + datas);
              this.router.navigate(['/tabs/tab2', datas.id]);
              this.regdata = datas;
