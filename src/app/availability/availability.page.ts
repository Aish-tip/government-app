...
export class AvailabilityPage implements OnInit {
  ...
  uploadFile() {
    this.http.get(`${Urls.FILES}/${this.id}`).subscribe((res => {
      console.log(res)
      this.upload()
      ...
      if (err.statusText == "Not Found") {
        this.http.post(`${Urls.FILES}`, { "name": this.id }).subscribe((res => {
          console.log(res)
          this.upload()
        }))
      }
  ...
  upload() {
    this.http.post<{ imageUrl: string, imagePath: string }>(`${Urls.FILES}/${this.id}/upload`, this.formDataToUpload).subscribe(res => {
      console.log(res)
